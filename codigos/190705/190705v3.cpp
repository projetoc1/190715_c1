...
float SETPOINT(float tempo,float intervalo,float amplitude,float t_ref)
{
	float tempo_efetivo;
	float saida;
	int n;

	tempo_efetivo = fmod(tempo,intervalo);
	n = floor(tempo/intervalo);
	if (n%2 == 0)
	{
		if (n==0)
		{
			saida=amplitude*(1-exp(-(tempo_efetivo-t_ref*log(0.5))/t_ref));			
		}
		else
		{
			//setpoint = setpoint0*(1-exp(-rotina_temp_y/t_ref));
			saida=amplitude*(1-exp(-tempo_efetivo/t_ref));
		}
	}
	else
	{
		saida=(amplitude - amplitude*(1-exp(-tempo_efetivo/t_ref)));
	}
	saida=saida-amplitude/2;
	return saida;
}

float SETPOINT2(float tempo,float intervalo,float amplitude,float t_ref)
{
	float tempo_efetivo;
	float saida;
	int n;
...
int main()
{
    ...
    while(true)
    {
    	...
    	{	
        	...
        	rotina_temp = rotina_temp + dt; //contador de tempo da rotina
            rotina_temp_y= rotina_temp_y + dt;
        	rotina_temp_z= rotina_temp_z + dt;

        	//Rotina de setpoints
        	if (rotina_temp>=20) //10 segundos de intervalo entre setpoints
        	{
        		rotina_temp = 0;
        		rotina = rotina + 1;
        		if (rotina==4)
        	...
        	//(-20,-20),(20,-20),(20,20),(-20,20) repetição
        	if (rotina==0)
        	{
        		setpoint0   = grau_min;
        		setpoint0z  = grau_min;
            ...
            setpoint = SETPOINT2(segundos,40,30,t_ref);
            ...
            //setpointz = setpoint0z*(1-exp(-rotina_temp/t_ref));
            //setpointz = SETPOINT(segundos,40,40,t_ref);
            setpointz = SETPOINT2(segundos,40,30,t_ref);
        ...
        printf("setpoint=%.2f setpointz = %.2f tempo da rotina = %.2f\n",setpoint,setpointz,rotina_temp);
    }
    return 0;
}